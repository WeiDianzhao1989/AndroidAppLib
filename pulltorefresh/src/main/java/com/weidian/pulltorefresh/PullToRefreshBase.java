...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	protected boolean mIsFinishAutoLoading = false;
	protected boolean mIsPauseAutoLoading = false;
	...
	@Override
    public final boolean isAutoLoadingFinish() {
        return mState == State.FINISH_AUTO_LOADING || mIsFinishAutoLoading;
    }

	@Override
	public final boolean isAutoLoadingPause() {
		return mState == State.PAUSE_AUTO_LOADING || mIsPauseAutoLoading;
	}
				...
				if (isReadyForPull()) {
					...
					if (absDiff > mTouchSlop && (!mFilterTouchEvents || absDiff > Math.abs(oppositeDiff))) {
						if (mMode.showHeaderLoadingLayout() && diff >= 1f && isReadyForPullStart()) {
                            ...
                            if(!isRefreshing()) {
                                if (mMode == Mode.BOTH || mMode == Mode.PULL_START_AND_AUTO_REFRESH) {
	...
	public final void onRefreshComplete() {
		...
		boolean isAutoLoadingPause = isAutoLoadingPause();
		boolean isAutoLoadingFinish = isAutoLoadingFinish();

		if (isRefreshing || isAutoLoadingPause || isAutoLoadingFinish) {

			mIsPauseAutoLoading = false;
			mIsFinishAutoLoading = false;
			setState(State.RESET);
            if(mCurrentMode == Mode.AUTO_LOAD &&
                    mMode == Mode.PULL_START_AND_AUTO_REFRESH){
                mCurrentMode = Mode.PULL_FROM_START;
            }

			//恢复下拉刷新控件从REFRESHING->RESET后给予一个回调，有机会做额外处理
			if(isRefreshing && (mMode == Mode.AUTO_LOAD
					|| mMode == Mode.PULL_START_AND_AUTO_REFRESH)) {
				post(new Runnable() {
					@Override
					public void run() {
    ...
    @Override
    public final void pauseAutoLoading(){
        if(isRefreshing()) {
            setState(State.PAUSE_AUTO_LOADING);
			if(mCurrentMode == Mode.AUTO_LOAD){
				mCurrentMode = Mode.PULL_FROM_START;
			}
        }
    ...
    @Override
    public final void onAutoLoadingFinish() {
        if(isRefreshing()) {
            setState(State.FINISH_AUTO_LOADING);
			if(mCurrentMode == Mode.AUTO_LOAD &&
					mMode == Mode.PULL_START_AND_AUTO_REFRESH){
				mCurrentMode = Mode.PULL_FROM_START;
			}
    ...
    public final void onLoadingMore(){
        if(mMode == Mode.AUTO_LOAD || mMode == Mode.PULL_START_AND_AUTO_REFRESH){
            if (!isRefreshing() && !isAutoLoadingFinish() && !isAutoLoadingPause()) {
                mCurrentMode = Mode.AUTO_LOAD;
		...
		switch (event.getAction()) {
			...
			case MotionEvent.ACTION_UP: {
				if (mIsBeingDragged) {
					...
					setState(State.RESET);
					if(mIsFinishAutoLoading){
						setState(State.FINISH_AUTO_LOADING);
					}else if(mIsPauseAutoLoading){
						setState(State.PAUSE_AUTO_LOADING);
					}
	...
	protected abstract void onResetFinish();
	...
	protected void onReset() {
		...
		mHeaderLayout.reset();
		if(mMode != Mode.PULL_START_AND_AUTO_REFRESH && mMode != Mode.AUTO_LOAD) {
			mFooterLayout.reset();
		}
	...
	protected final void onRestoreInstanceState(Parcelable state) {
		if (state instanceof Bundle) {
			...
			Mode tempMode = Mode.mapIntToValue(bundle.getInt(STATE_CURRENT_MODE, 0));
			if(tempMode != mCurrentMode){
				// 有可能是onSaveInstanceState又改变当前模式，此时暂时处理方式是依据最新改的mode
			}else {
            ...
            if(!isAutoLoadingFinish() && !isAutoLoadingPause()) {
	...
	protected void updateUIForMode() {
		...
		// set it to pull down
		mCurrentMode = (mMode != Mode.BOTH && mMode != Mode.PULL_START_AND_AUTO_REFRESH) ? mMode : Mode.PULL_FROM_START;
	}
	...
	private boolean isReadyForPull() {
		switch (mMode) {
			case PULL_FROM_START:
            case PULL_START_AND_AUTO_REFRESH:
	...
	public enum Mode {
        ...
        PULL_START_AND_AUTO_REFRESH(0x6);
		/**
		...
		boolean permitsPullToRefresh() {
			return !(this == DISABLED || this == MANUAL_REFRESH_ONLY || this == AUTO_LOAD);
		}
		...
		public boolean showHeaderLoadingLayout() {
			return this == PULL_FROM_START || this == BOTH || this == PULL_START_AND_AUTO_REFRESH;
		}
	...
	public enum State {
        ...
        PAUSE_AUTO_LOADING(0x11),

        FINISH_AUTO_LOADING(0x12);
