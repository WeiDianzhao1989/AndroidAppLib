		...
		if (view.isPullToRefreshOverScrollEnabled() && !view.isRefreshing()) {
			...
			if (mode.permitsPullToRefresh() && !isTouchEvent && deltaValue != 0) {
				...
				if (newScrollValue < (0 - fuzzyThreshold)) {
                    ...
                    if(view.isReadyForPullStart() || !view.isAutoLoadingFinish() || !view.isAutoLoadingPause()) {
                        view.setState(State.RESET);
						if(view.isAutoLoadingFinish()){
							view.setState(State.FINISH_AUTO_LOADING);
						}else if(view.isAutoLoadingPause()){
							view.setState(State.PAUSE_AUTO_LOADING);
						}
                    }
				}
                ...
                if(view.isReadyForPullStart() || !view.isAutoLoadingFinish()
						|| !view.isAutoLoadingPause()) {
                    view.setState(State.RESET);
					if(view.isAutoLoadingFinish()){
						view.setState(State.FINISH_AUTO_LOADING);
					}else if(view.isAutoLoadingPause()){
